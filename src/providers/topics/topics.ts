...
export class TopicsProvider {
  ...
  private topicsMap: Map<string|number,Array<{}>> = new Map();
  ...
  public refreshTopics(category: {},continuation:boolean = false): Promise<any> {
    ...
    let topics = this.topicsMap.get(category['id'])||[];

    let refreshPromise = new Promise((resolve,reject) => {
      this.applicationSettings.getApplicationSettings()
        .then((applicationSettings) => {

          if(!continuation) topics = [];

          this.serviceClient
            .getTopics(category,1,topics.length,applicationSettings.language)
            .then((_topics) => {

              if(!topics) topics = [];

              _topics = this.preprocessTopics(_topics); 
              this.formatDateAndTimeForTopics(_topics);
              this.orderTopicsChronologically(_topics);

              let prevTopics = topics;
              topics = continuation ? topics.concat(_topics) : _topics;

              let topicsUpdatedInfo = new TopicsUpdatedInfo(category, topics, topics.length ,
              continuation ? topics.length > prevTopics.length :topics.length > 0,continuation);
            
              this.topicsMap.set(category['id'],topics);
  ...
  public getTopics(category): Array<any> {
    if (this.getOnlyHotTopics)
      return this.filterHotTopics(category);
    else {
      return ((category&&this.topicsMap.get(category['id']))||[]).slice(0);
    }
  }
  ...
  public setSelectedTopic(category:{}, topic: any) {
    //console.log('topics setSelectedTopic: ',category,topic);
  ...
  public loadNextTopic(category: {}, currentTopic: {}, isSearch: boolean, shouldForceNextCategory: boolean = false) {
    let existingTopics = isSearch ? this.topicsByKeyword : this.getTopics(category);
    ...
    else this.setSelectedTopic(category, existingTopics[index + 1]);
    return true;
  }
  ...
  private filterHotTopics(category): Array<{}> {
    let topicsCopy = ((category&&this.topicsMap.get(category['id']))||[]).slice(0);
